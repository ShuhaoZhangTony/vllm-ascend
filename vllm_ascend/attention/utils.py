...
def _split_decode_prefill_boundary_host(
    ...
    """Find the boundary from CPU metadata with one batched conversion."""
...
def _split_decode_prefill_boundary(
    ...
    *,
    ...
    # CommonAttentionMetadata stores these values on the CPU. A host scan is
    # faster for normal scheduler sizes; retain tensor operations for the tail.
    ...
    if require_uniform:
        ...
        is_prefill = torch.where(force_all_decode, torch.zeros_like(is_prefill), is_prefill)
        is_prefill = torch.where(first_is_prefill, torch.ones_like(is_prefill), is_prefill)
    else:
        is_prefill = query_lens > decode_threshold
